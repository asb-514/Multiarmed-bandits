...
Original file is located at
    https://colab.research.google.com/drive/1lcFHh8tJys3L3_lg-YIVvnbls0ZvrwkS
"""
...
class EE21B021_NUMBER_LINUCB:
    def __init__(self, num_dim, num_samples, sigma,arm):
        ...
        self.__sigma = sigma
        self.__arms = arm
        self.myV = np.array([0]*self.__num_dim*self.__num_dim,dtype = float).reshape(self.__num_dim,self.__num_dim)
        self.lamda = 0.001
    ...
    def get_action(self, reward):
        ...
        for i in range(len(self.__arms)):
            if self.myarmtimes[i] <= 1:
                ...
                return now
        beta = np.sqrt(self.lamda * self.__num_dim) + np.sqrt(2*np.log(self.time ** 1) + self.__num_dim*np.log( 1 + (self.time/(self.lamda))))
        for i in range(len(self.__arms)) :
