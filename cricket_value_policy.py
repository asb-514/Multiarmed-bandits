# -*- coding: utf-8 -*-
"""cricket-value-policy.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/17yILXYwnmNY9ZkfFH-LtFSZ3OHTPq7JW
"""
...
def Calculate_Value(V_in):
	...
	for i in range(1,np.size(V_in)):
		for a in range(np.size(runs)):
			...
			Q_out[i][a]=(1-p_w) * ( p_r*(runs[a]+V_in[i]) + (1-p_r)* (0+V_in[i]) ) + p_w*V_in[i-1]
...
PI[max_balls-1,:],V[max_balls-1,:],Q[max_balls-1,:] = Calculate_Value(np.zeros((max_wickets),dtype=float))
